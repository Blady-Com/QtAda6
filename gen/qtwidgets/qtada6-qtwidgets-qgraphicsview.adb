...
package body QtAda6.QtWidgets.QGraphicsView is
   ...
   function rubberBandChanged (self : access Inst) return CLASSVAR_Signal is
   begin
      return
   ...
   begin
      ...
      Args   := Tuple_New (2);
      Tuple_SetItem (Args, 0, (if item_P /= null then item_P.Python_Proxy else No_Value));
      Tuple_SetItem (Args, 1, (if aspectRadioMode_P /= null then aspectRadioMode_P.Python_Proxy else No_Value));
      Result := Object_CallObject (Method, Args, True);
   end fitInView;
   procedure fitInView
   ...
   begin
      ...
      Tuple_SetItem (Args, 2, Float_FromDouble (w_P));
      Tuple_SetItem (Args, 3, Float_FromDouble (h_P));
      Tuple_SetItem (Args, 4, (if aspectRadioMode_P /= null then aspectRadioMode_P.Python_Proxy else No_Value));
      Result := Object_CallObject (Method, Args, True);
   end fitInView;
   ...
   procedure updateSceneRect (self : access Inst; rect_P : UNION_QtAda6_QtCore_QRectFQtAda6_QtCore_QRect) is
      Method, Args, List, Result : Handle;
   begin
