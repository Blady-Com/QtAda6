...
package body QtAda6.QtWidgets.QMessageBox is
   ...
   begin
      ...
      Tuple_SetItem (Args, 2, Unicode_FromString (text_P));
      Tuple_SetItem (Args, 3, (if buttons_P /= null then buttons_P.Python_Proxy else No_Value));
      Tuple_SetItem (Args, 4, (if defaultButton_P /= null then defaultButton_P.Python_Proxy else No_Value));
      Result           := Object_CallObject (Method, Args, True);
      Ret.Python_Proxy := Result;
      return Ret;
   end critical;
   ...
   procedure setIconPixmap (self : access Inst; pixmap_P : UNION_QtAda6_QtGui_QPixmapQtAda6_QtGui_QImagestr) is
      Method, Args, List, Result : Handle;
   begin
