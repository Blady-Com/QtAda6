-------------------------------------------------------------------------------
-- NAME (body)                  : qtada6-qtcore-qxmlstreamnamespacedeclaration.adb
...
package body QtAda6.QtCore.QXmlStreamNamespaceDeclaration is
   use type QtAda6.int;
   ...
   begin
      Class := Object_GetAttrString (QtAda6.QtCore_Python_Proxy, "QXmlStreamNamespaceDeclaration");
      Args  := Tuple_New (0);
      Dict  := Dict_New;
   ...
   begin
      ...
      Tuple_SetItem
        (Args, 0,
         (if QXmlStreamNamespaceDeclaration_P /= null then QXmlStreamNamespaceDeclaration_P.Python_Proxy
          else No_Value));
      Dict := Dict_New;
      return new Inst'(Python_Proxy => Object_Call (Class, Args, Dict, True));
   end Create;
   function Create (prefix_P : str; namespaceUri_P : str) return Class is
      Class, Args, Dict, List, Tuple : Handle;
   begin
      Class := Object_GetAttrString (QtAda6.QtCore_Python_Proxy, "QXmlStreamNamespaceDeclaration");
      Args  := Tuple_New (2);
      Tuple_SetItem (Args, 0, Unicode_FromString (prefix_P));
      Tuple_SetItem (Args, 1, Unicode_FromString (namespaceUri_P));
      Dict := Dict_New;
      return new Inst'(Python_Proxy => Object_Call (Class, Args, Dict, True));
   end Create;
