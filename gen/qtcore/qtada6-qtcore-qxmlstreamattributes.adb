...
package body QtAda6.QtCore.QXmlStreamAttributes is
   ...
   begin
      Class := Object_GetAttrString (QtAda6.QtCore_Python_Proxy, "QXmlStreamAttributes");
      Args  := Tuple_New (1);
      Tuple_SetItem (Args, 0, QXmlStreamAttributes_P.Python_Proxy);
      return new Inst'(Python_Proxy => Object_CallObject (Class, Args, True));
   end Create;
   function U_add_U
   ...
   function mid (self : access Inst; pos_P : int; len_P : int) return List_QtAda6_QtCore_QXmlStreamAttribute is
      Method, Args, Result : Handle;
   begin
