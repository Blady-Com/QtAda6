...
package body QtAda6.QtCore.QFutureInterfaceBase is
   ...
   function Create (initialState_P : access QtAda6.QtCore.QFutureInterfaceBase.State.Inst'Class) return Class is
      Class, Args : Handle;
   begin
      Class := Object_GetAttrString (QtAda6.QtCore_Python_Proxy, "QFutureInterfaceBase");
      Args  := Tuple_New (1);
      Tuple_SetItem (Args, 0, initialState_P.Python_Proxy);
   ...
   begin
      ...
      Args   := Tuple_New (1);
      Tuple_SetItem (Args, 0, pool_P.Python_Proxy);
      Result := Object_CallObject (Method, Args, True);
   end setThreadPool;
