...
package body QtAda6.QtGui.QStandardItemModel is
   ...
   procedure setItemRoleNames (self : access Inst; roleNames_P : DICT_intQtAda6_QtCore_QByteArray) is
      Method, Args, List, Result : Handle;
   begin
      Method := Object_GetAttrString (self.Python_Proxy, "setItemRoleNames");
      Args   := Tuple_New (1);
      Tuple_SetItem (Args, 0, (if roleNames_P /= null then roleNames_P.Python_Proxy else No_Value));
      Result := Object_CallObject (Method, Args, True);
