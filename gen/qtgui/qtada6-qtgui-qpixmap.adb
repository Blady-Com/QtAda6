...
package body QtAda6.QtGui.QPixmap is
   ...
   begin
      Method := Object_GetAttrString (self.Python_Proxy, "convertFromImage");
      Args   := Tuple_New (1);
      Tuple_SetItem (Args, 0, Unicode_FromString (img_P));
      Dict := Dict_New;
      if flags_P /= null then
         Dict_SetItemString (Dict, "flags", flags_P.Python_Proxy);
      end if;
   ...
   begin
      Method := Object_GetAttrString (self.Python_Proxy, "createMaskFromColor");
      Args   := Tuple_New (1);
      Tuple_SetItem (Args, 0, Unicode_FromString (maskColor_P));
      Dict := Dict_New;
      if mode_P /= null then
         Dict_SetItemString (Dict, "mode", mode_P.Python_Proxy);
      end if;
   ...
   begin
      Method := Object_GetAttrString (self.Python_Proxy, "createMaskFromColor");
      Args   := Tuple_New (1);
      Tuple_SetItem (Args, 0, Long_FromLong (maskColor_P));
      Dict := Dict_New;
      if mode_P /= null then
         Dict_SetItemString (Dict, "mode", mode_P.Python_Proxy);
      end if;
   ...
   procedure fill (self : access Inst; fillColor_P : access QtAda6.QtGui.QColor.Inst'Class := null) is
      Method, Args, Dict, List, Tuple, Set, Result : Handle;
   begin
   ...
   procedure fill (self : access Inst; fillColor_P : access QtAda6.QtGui.QRgba64.Inst'Class := null) is
      Method, Args, Dict, List, Tuple, Set, Result : Handle;
   begin
      Method := Object_GetAttrString (self.Python_Proxy, "fill");
      Args   := Tuple_New (0);
   ...
   procedure fill (self : access Inst; fillColor_P : Any := null) is
      Method, Args, Dict, List, Tuple, Set, Result : Handle;
   begin
      Method := Object_GetAttrString (self.Python_Proxy, "fill");
      Args   := Tuple_New (0);
   ...
   procedure fill (self : access Inst; fillColor_P : access QtAda6.QtCore.Qt.GlobalColor.Inst'Class := null) is
      Method, Args, Dict, List, Tuple, Set, Result : Handle;
   begin
      Method := Object_GetAttrString (self.Python_Proxy, "fill");
      Args   := Tuple_New (0);
   ...
   procedure fill (self : access Inst; fillColor_P : str := "") is
      Method, Args, Dict, List, Tuple, Set, Result : Handle;
   begin
      Method := Object_GetAttrString (self.Python_Proxy, "fill");
      Args   := Tuple_New (0);
      Dict   := Dict_New;
      if fillColor_P /= "" then
         Dict_SetItemString (Dict, "fillColor", Unicode_FromString (fillColor_P));
      end if;
      Result := Object_Call (Method, Args, Dict, True);
   end fill;
   procedure fill (self : access Inst; fillColor_P : int := 0) is
      Method, Args, Dict, List, Tuple, Set, Result : Handle;
   begin
      Method := Object_GetAttrString (self.Python_Proxy, "fill");
      Args   := Tuple_New (0);
      Dict   := Dict_New;
      if fillColor_P /= 0 then
         Dict_SetItemString (Dict, "fillColor", Long_FromLong (fillColor_P));
      end if;
      Result := Object_Call (Method, Args, Dict, True);
   end fill;
   function fromImage
   ...
   begin
      ...
      Args   := Tuple_New (1);
      Tuple_SetItem (Args, 0, Bytes_FromString (Standard.String (buf_P.all)));
      Dict := Dict_New;
      if format_P /= null then
         Dict_SetItemString (Dict, "format", Bytes_FromString (Standard.String (format_P.all)));
      end if;
   ...
   begin
      ...
      Tuple_SetItem (Args, 2, (if rect_P /= null then rect_P.Python_Proxy else No_Value));
      Dict := Dict_New;
      if exposed_P /= NoneType_default_is_not_supported then
         Dict_SetItemString (Dict, "exposed", NoneType_conv_A2P_is_not_supported);
      end if;
      Result := Object_Call (Method, Args, Dict, True);
   end scroll;
   procedure scroll
