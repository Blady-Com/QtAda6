...
begin
  Class := Object_GetAttrString (QtAda6.QtGui_Python_Proxy, "QConicalGradient");
  Args   := Tuple_New ( 1);
  Tuple_SetItem (Args, 0,QConicalGradient_P.Python_Proxy);
  return new Inst'(Python_Proxy => Object_CallObject (Class, Args, True));
end;
function Create(center_P : Union_QtAda6_QtCore_QPointF_QtAda6_QtCore_QPoint_QtAda6_QtGui_QPainterPath_Element;startAngle_P : float) return Class is
  Class, Args : Handle;
begin
  Class := Object_GetAttrString (QtAda6.QtGui_Python_Proxy, "QConicalGradient");
  Args   := Tuple_New ( 2);
