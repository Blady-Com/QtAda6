...
package body QtAda6.QtGui.QAbstractTextDocumentLayout is
   ...
   function documentSizeChanged (self : access Inst) return access QtAda6.QtCore.Signal.Inst'Class is
   begin
      return
   ...
   function pageCountChanged (self : access Inst) return access QtAda6.QtCore.Signal.Inst'Class is
   begin
      return
        new QtAda6.QtCore.Signal.Inst'(Python_Proxy => Object_GetAttrString (self.Python_Proxy, "pageCountChanged"));
   end pageCountChanged;
   function update (self : access Inst) return access QtAda6.QtCore.Signal.Inst'Class is
   begin
      return new QtAda6.QtCore.Signal.Inst'(Python_Proxy => Object_GetAttrString (self.Python_Proxy, "update"));
   end update;
   function updateBlock (self : access Inst) return access QtAda6.QtCore.Signal.Inst'Class is
   begin
   ...
   begin
      ...
      Tuple_SetItem (Args, 0, Long_FromLong (objectType_P));
      Dict := Dict_New;
      if component_P /= null then
         Dict_SetItemString (Dict, "component", component_P.Python_Proxy);
      end if;
      Result := Object_Call (Method, Args, Dict, True);
