...
package body QtAda6.QtGui.QMatrix4x4 is
   ...
   procedure ortho (self : access Inst; rect_P : access QtAda6.QtCore.QRectF.Inst'Class) is
      Method, Args, Dict, List, Tuple, Set, Result : Handle;
   begin
      Method := Object_GetAttrString (self.Python_Proxy, "ortho");
      Args   := Tuple_New (1);
   ...
   begin
      ...
      Dict := Dict_New;
      if z_P /= 0.0 then
         Dict_SetItemString (Dict, "z", Float_FromDouble (z_P));
      end if;
      Result := Object_Call (Method, Args, Dict, True);
   end rotate;
   ...
   begin
      ...
      Dict := Dict_New;
      if nearPlane_P /= 0.0 then
         Dict_SetItemString (Dict, "nearPlane", Float_FromDouble (nearPlane_P));
      end if;
      if farPlane_P /= 0.0 then
         Dict_SetItemString (Dict, "farPlane", Float_FromDouble (farPlane_P));
      end if;
      Result := Object_Call (Method, Args, Dict, True);
   end viewport;
   procedure viewport (self : access Inst; rect_P : access QtAda6.QtCore.QRectF.Inst'Class) is
      Method, Args, Dict, List, Tuple, Set, Result : Handle;
   begin
      Method := Object_GetAttrString (self.Python_Proxy, "viewport");
      Args   := Tuple_New (1);
   ...
   procedure viewport (self : access Inst; rect_P : access QtAda6.QtCore.QRect.Inst'Class) is
      Method, Args, Dict, List, Tuple, Set, Result : Handle;
   begin
      Method := Object_GetAttrString (self.Python_Proxy, "viewport");
      Args   := Tuple_New (1);
